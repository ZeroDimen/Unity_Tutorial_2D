...
public class Transform_LoopObject : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (this.gameObject.transform.position.x <= -Return_Pos_X) // 오브젝트 x축 위치가 -Return_Pos.x 이하일경우
        {
            Random_Pos_Y = Random.Range(-8f, -4.5f);
            // 오브젝트 x축 위치를 -Return_Pos.x로 이동
            // 해결방법: Time.deltaTime의 오차를 포함하는 좌표에 오브젝트 이동
            this.gameObject.transform.position = new Vector3(Return_Pos_X * 2 + this.gameObject.transform.position.x, Random_Pos_Y, 0f);
        }
    }
