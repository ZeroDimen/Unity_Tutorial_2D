...
namespace Cat
{
    public class Cat_UIManager : MonoBehaviour
    {
        ...
        public void Ending(string Ending)
        {
            ...
            {
                Debug.Log("Err -> Cat_UIManager : Ending(string Ending)");
            }
            
            // Invoke("Video(Ending)",5f); Invoke로는 호출하는 함수에게 매개변수를 넘겨줄수없음
            // 따라서 매개변수를 사용하는 함수를 지연호출하기위해서는 Courutine을 사용해야함
            StartCoroutine(Video(Ending,3f));
        }

        IEnumerator Video(string Ending,float delay)
        {
